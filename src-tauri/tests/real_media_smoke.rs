...
fn fx() -> Option<&'static Env> {
    E.get_or_init(|| {
        ...
        let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("..").join("video");
        ...
        Some(Env { ffmpeg, ffprobe, dir, a, b, small, big })
    })
    .as_ref()
}
...
fn run(e: &Env, args: &[String], what: &str) -> Duration {
    let t = Instant::now();
    let out = Command::new(&e.ffmpeg).args(args).output().expect("ffmpeg 启动失败");
    let d = t.elapsed();
    assert!(out.status.success(), "{what} 失败：{}", String::from_utf8_lossy(&out.stderr));
...
fn ffprobe_json(e: &Env, p: &Path) -> Value {
    let out = Command::new(&e.ffprobe)
        ...
        .output()
        .expect("ffprobe 启动失败");
    assert!(out.status.success(), "ffprobe 失败：{}", String::from_utf8_lossy(&out.stderr));
...
fn streams_of<'a>(v: &'a Value, kind: &str) -> Vec<&'a Value> {
    v["streams"]
        .as_array()
        .map(|a| a.iter().filter(|s| s["codec_type"].as_str() == Some(kind)).collect())
        .unwrap_or_default()
}

fn duration_of(v: &Value) -> f64 {
    v["format"]["duration"].as_str().unwrap_or("0").parse().unwrap_or(0.0)
}

fn video_dims(v: &Value) -> (i64, i64) {
    let st = streams_of(v, "video");
    let f = st.first().expect("无视频流");
    (f["width"].as_i64().unwrap_or(0), f["height"].as_i64().unwrap_or(0))
}
...
fn display_rotation(v: &Value) -> Option<f64> {
    streams_of(v, "video")
        ...
        .as_array()?
        .iter()
        .find(|d| d["side_data_type"].as_str().map(|x| x.contains("Display Matrix")).unwrap_or(false))?
        .get("rotation")?
        .as_f64()
}
...
fn first_video_pts(e: &Env, p: &Path) -> f64 {
    let out = Command::new(&e.ffprobe)
        ...
        .output()
        .unwrap();
    String::from_utf8_lossy(&out.stdout).lines().next().unwrap_or("").trim().parse().unwrap_or(f64::NAN)
}
...
fn assert_decodable(e: &Env, p: &Path) {
    ...
    println!("  [ok] 全帧解码 {}  {:.2}s", p.file_name().unwrap().to_string_lossy(), t.elapsed().as_secs_f64());
}

/// 解码某个时间窗口，专抓 concat 接缝处的时间戳损坏。
fn assert_decodable_window(e: &Env, p: &Path, from: f64, dur: f64) {
    let out = Command::new(&e.ffmpeg)
        .args(["-v", "error", "-ss", &format!("{from:.3}"), "-t", &format!("{dur:.3}"), "-i"])
        .arg(p)
        .args(["-f", "null", "-"])
        .output()
        .unwrap();
    ...
    let fatal: Vec<_> = stderr
        ...
        .filter(|l| !l.contains("non monotonically increasing dts to muxer"))
        .collect();
    assert!(out.status.success() && fatal.is_empty(), "窗口 {from:.2}~{:.2} 解码失败：{}", from + dur, fatal.join("\n"));
}
...
fn t1_real_lossless_merge_and_seam() {
    ...
    run(e, &cmd::concat_args(&s(&list), &s(&merged)), "concat 无损合并（210MB 源）");

    let v = ffprobe_json(e, &merged);
    let got = duration_of(&v);
    println!("  实测：{got:.3}s  大小={:.1}MB", merged.metadata().unwrap().len() as f64 / 1e6);
    assert!((got - (da + db)).abs() <= 0.6, "时长 {got:.3}s 偏离 {:.3}±0.6s", da + db);

    let (vs, asu) = (streams_of(&v, "video"), streams_of(&v, "audio"));
    assert_eq!(vs.len(), 1, "视频流数量应保持 1");
    assert_eq!(asu.len(), 1, "音频流数量应保持 1");
    assert_eq!(vs[0]["codec_name"].as_str(), Some("h264"), "视频编码应原样 copy");
    assert_eq!(asu[0]["codec_name"].as_str(), Some("aac"), "音频编码应原样 copy");
...
fn t2_real_fast_cut_snaps_within_one_gop() {
    ...
    println!("  极乐净土：{} 个关键帧，最大 GOP={max_gop:.3}s，首帧={:.3}", kfs.len(), kfs[0]);

    let t = run(e, &cmd::cut_args(30.0, 5.0, &s(&e.big), &s(&out)), "极速剪切 30.0s+5.0s（1GB 源）");
    ...
    assert_eq!(streams_of(&v, "video")[0]["codec_name"].as_str(), Some("h264"), "极速剪切不得重编码");
    assert_eq!(video_dims(&v), (1920, 1080));

    // 时长下界：源里 30.0 前的最近关键帧；上界：请求时长 + 一个 GOP 余量
    let lower = 5.0 - max_gop - 0.5;
    assert!(got >= lower && got <= 5.5, "时长 {got:.3}s 超出 [{lower:.3}, 5.5]（最大 GOP={max_gop:.3}s）");
...
fn t3_real_precise_cut_is_accurate() {
    ...
    run(
        e,
        &cmd::precise_cut_args(2.0, 3.0, &s(&e.small), &s(&out), "libx264", QualityPreset::Balanced),
        "精确剪切 2.0s+3.0s",
    );
    let v = ffprobe_json(e, &out);
    let got = duration_of(&v);
...
fn t4_real_rotate_remux_is_pixel_identity() {
    ...
    run(e, &cmd::rotate_remux_args_deg(90, false, false, &s(&e.a), &s(&out)), "元数据旋转 90°（remux）");

    let after = ffprobe_json(e, &out);
    assert_eq!(video_dims(&after), video_dims(&before), "remux 不应改变编码分辨率");
    assert_eq!(display_rotation(&after), Some(90.0), "应写入 Display Matrix rotation=90");
    assert_eq!(streams_of(&after, "audio").len(), 1);
    assert_eq!(streams_of(&after, "video")[0]["codec_name"].as_str(), Some("h264"));
    let (db, da) = (duration_of(&before), duration_of(&after));
    assert!((da - db).abs() <= 0.05, "remux 时长应不变：{db:.3}s → {da:.3}s");
    assert_decodable(e, &out);
}
...
fn t5_real_rotate_transcode_swaps_dims() {
    ...
    run(
        e,
        &cmd::rotate_transcode_args(90, false, false, &s(&e.small), &s(&out), "libx264", QualityPreset::Balanced),
        "重编码旋转 90°",
    );
    let v = ffprobe_json(e, &out);
...
fn t6_real_workbench_pipeline_end_to_end() {
    ...
    run(e, &cmd::pipeline_copy_args(Some((0.0, 6.0)), 0, false, false, &s(&e.a), &s(&seg1)), "片段1 copy 0..6s");
    ...
    run(e, &cmd::concat_args(&s(&list), &s(&final_out)), "成品 concat");

    let v = ffprobe_json(e, &final_out);
    let total = duration_of(&v);
    println!("  片段 {d1:.3}s + {d2n:.3}s = {:.3}s，成品 {total:.3}s", d1 + d2n);
    assert!((total - (d1 + d2n)).abs() <= 0.6, "成品时长 {total:.3}s 偏离 {:.3}±0.6s", d1 + d2n);
...
fn t7_real_big_probe_is_correct_and_cached() {
    ...
    println!("  1GB 探测：{d1:.3}s 用时 {:.1}ms → {d2:.3}s 用时 {:.1}ms", e1.as_secs_f64() * 1e3, e2.as_secs_f64() * 1e3);
    assert!((d1 - 264.554).abs() <= 0.5, "时长 {d1:.3}s 偏离 264.554");
    assert_eq!(d1, d2, "两次探测结果应一致");
    assert!(e2 <= e1 + Duration::from_millis(5), "第二次探测应命中缓存（{e1:?} → {e2:?}）");

    let facts = probe::probe_merge_facts_sync(&e.ffprobe, &path).unwrap();
    assert_eq!((facts.info.video.width, facts.info.video.height), (1920, 1080));
...
fn t8_real_keyframe_parse_completeness() {
    ...
    println!("  ffprobe 原始 {} 行，解析得到 {} 行；带多余字段的行={:?}", raw.len(), parsed.len(), comma_lines);
    println!("  首帧：ffprobe={} 解析后={:?}", raw[0], parsed.first());
    println!("  扫描耗时 {:.2}s", scan.as_secs_f64());

    assert!(!comma_lines.is_empty(), "本素材不应出现多余字段（前提变了，用例需重写）");
    assert_eq!(
        parsed.len(),
        raw.len(),
        ...
        parsed.len(),
        raw.iter()
            .filter(|l| l.split(',').next().unwrap_or("").trim().parse::<f64>().is_ok())
            .filter(|l| l.trim().parse::<f64>().is_err())
            .collect::<Vec<_>>()
    );
    assert_eq!(parsed.first().copied(), Some(0.0), "首个关键帧应为 0.0");
    assert!(parsed.windows(2).all(|w| w[1] > w[0]), "关键帧时间点应严格升序");
