...
> **最后更新**：2026-09-25（同步 `M11-5`/`M11-6`/`M11-7`（切割/边缘修剪/撤销接线 ✅；FR-17xx 行补 `FR-1736`/`FR-1737` 发号、`FR-1735` 修订与 `TC-041`）；同日此前：同步 `M11-3`/`M11-4`（播放头路径 ✅、选择与波纹删除 ✅）、同步 `M11-1`/`M11-2`、登记第三轮审查（`T-005`/`T-006` 立项）、同步 `R2-1`~`R2-3` 与 `M11-0`/`R3-7`；此前 2026-09-23：同步 R4 段实施完毕 `R4-1`–`R4-9` 与回归 `TC-019`–`TC-029`）
...
| **FR-17xx** 单轨时间线（§17.3 坐标与精度随 `M11-1` 发号：`FR-1730`~`FR-1735`——`FR-1735` 随 `M11-6`/`BUG-012` 修订为 `max(1帧, 0.05s)`；§17.5 撤销栈随 `M11-7` 发号：`FR-1736`/`FR-1737`，见 [TIMELINE.md](./TIMELINE.md) §17.3/§17.5；其余 §17.x 随后续批次发号） | TIMELINE.md §17 | `ClipTimeline`/`CropOverlay` · `ProductPreview`/`TaskProgress`/`hooks` | M11-0 ✅ … M11-7 ✅ · M11-8 · M11-9 · M12 · M13 | TIMELINE.md §17.9 ①~⑥ · **TC-040**（撤销基线，`M11-0`）· **TC-041**（BUG-012 回归 + 修剪几何，`M11-6`）· 其余 TC 待建 | 🔄 进行中（`M11-0`–`M11-7` 已实施） |
