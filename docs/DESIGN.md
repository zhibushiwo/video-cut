...
> 版本：v0.5 · 更新日期：2026-09-14 · 状态：v0.2 已实现（M0–M5、M4-1/2）；工作台 2.0（M6）、设置二期（M4-8）、反馈批次（M7）已实现待手测；功能候选池 §16 未排期
>
...
| # | 候选 | 说明 | 量级 |
| --- | --- | --- | --- |
| B1 | probe / 关键帧索引缓存 | 按 `路径+size+mtime` 缓存 MediaInfo 与关键帧列表；重复打开同一文件秒出（现每次进剪切页重扫数秒） | 小 |
| B2 | 磁盘空间预检推广到全部任务 | cut 已有 fs4 预检（§8.2），抽公共函数接入 merge/rotate/crop/pipeline——空间不足不再编码中途失败留半成品 | 小 |
...
| # | 候选 | 说明 | 量级 |
| --- | --- | --- | --- |
| B14 | 核心链路 e2e 自动化 | 用 gen-fixtures 夹具真实跑「剪切→合并→pipeline」断言输出时长/可解码；防重构回归的保险 | 中 |
