...
- 冲突时由前端弹确认（覆盖 / 重命名 / 取消）；导出名默认追加时间戳防覆盖（M5），M4-8 起可在设置中关闭（关闭后同名输出走各页覆盖确认流程）
...
- 配置项（定义见 §12）：
  ...
  - **导出名加时间戳**（M4-8）：开关，默认开；控制各页导出是否自动追加时间戳后缀
...
- v1 配置项：默认输出目录、默认剪切模式（极速/精确）、关键帧吸附开关、代理预览开关/强制代理、编码器选择（自动/锁定）、质量档位；M4-8 追加：主题色（`accent`，预设枚举）、导出名时间戳开关（`appendTimestamp`，默认 true）
