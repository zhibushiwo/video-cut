...
> 版本：v0.6 · 更新日期：2026-09-14 · 状态：v0.2 已实现（M0–M5、M4-1/2）；工作台 2.0（M6）、设置二期（M4-8）、反馈批次（M7）已实现待手测；M8（候选池晋升批次 1：B1/B2/B14）实施中；候选池其余条目未排期
>
...
>
> **已晋升**：B1、B2、B14 → M8（2026-09-14，设计见 §6.5 / §6.6 / §8.2，决策 #22/#23）；下表保留原条目并标注去向。
...
| B17 | 深浅主题切换 | 亮色变量集整体替换 + 对比度校对（§14 阶段 4 既有项）；与主题色基建（§9.1）同源 | 中 |
