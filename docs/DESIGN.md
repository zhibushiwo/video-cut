...
**需求**：FR-380（组级）
**验收**：AC-380-1　——　执行：TESTING.md TC-003 · TC-013（原子需求随 M11 首个功能批次发号 FR-381+，见 [INDEX.md](./INDEX.md) §3.1 的发号口径）；口径以本节为准。
...
| 事件 | 时机 | payload |
| --- | --- | --- |
| `task-status` | 状态机迁移 | `{ taskId, status, error?, outputs, internal }`（`internal` = 内部任务，R3-3：事件照常派发，面板凭它跳过展示） |
...
}                               // internal 任务不进本列表（R3-3）；internal 标记在 task-status 的 payload 上
...
- 剪切：`<源文件名>_part_001.<扩展名>`——**扩展名随模式**（见下条：极速剪切跟随源容器、精确剪切固定 `.mp4`）；合并：用户指定；旋转/放大：`<源文件名>_rotated.<ext>` / `<源文件名>_zoomed.<ext>`（同样按下面的容器口径校正）
- 冲突时由前端弹确认（覆盖 / 重命名 / 取消）；用户命名的导出**默认不加时间戳，仅当目标已存在同名文件时自动追加**（M7-4，决策 #19）
- **容器与扩展名由命令决定，文件名必须与之一致**（[DECISIONS.md](./DECISIONS.md) `ADR-033`）：**copy 类**（极速剪切 / 无损合并 / 元数据旋转 / 全 copy 的工作台拼接）容器 = **源容器**、扩展名跟随源文件；**重编码类**（精确剪切 / 自动统一后合并 / 旋转转码 / 局部放大 / 含转码片段的工作台拼接）容器统一 **mp4**、扩展名固定 `.mp4`。用户或前端给的名字按上述规则**校正扩展名**（`fs::output_path_for`）；`.part` 半成品与最终产物**共用同一扩展名**（ffmpeg 靠扩展名推断封装）；校正后若与他人同名，按上一条追加时间戳而**不静默覆盖**
...
| 同名输出冲突 | 前端确认覆盖/重命名 |
| 输出路径与输入为**同一文件** | 拒绝（`fs::same_path`）：**提交期**查请求路径、**容器校正后**再查一次，两处共用同一实现。同一性按**归一化后**比较——大小写、末尾点/空格、`..` 绕行等写法都算同一文件；靠逐字符比较会放行这些写法，随后 `atomic_replace` 会替换掉**源文件**（`BUG-011`）。实测：`/` 与 `\` 混用、`./` 前缀本来就相等（Rust `Path` 按组件比较），真正会被逐字符比较放行的是上面几类 |
