...
> 版本：v0.7 · 更新日期：2026-09-15 · 状态：v0.2 已实现（M0–M5、M4-1/2）；工作台 2.0（M6）、设置二期（M4-8）、反馈批次（M7）、候选池批次 1（M8：B1/B2/B14）已实现（M6/M7 待手测复验）；M9/M10（工作台手测批次 2 修复 + 保活/深浅主题）已立项**待实施**；候选池其余条目未排期
>
...
- 缓存量级防御：条目数超过 512 整体清空（pipeline 中间文件路径每次任务都不同，条目极小但会缓慢累积，粗粒度清理足够）
...
- **磁盘空间预检（B2，M8 推广到全部输出型任务）**：检查在**任务作业线程内**执行（运行时而非提交时——排队期间磁盘状态可能变化）；空间不足直接 Failed 并提示，不再编码中途失败留半成品。估算规则（公共函数 `require_disk_space`，cut/rotate/crop/merge/pipeline/proxy 共用）：
...
**页面保活（M10-1，决策 #24）**：四个工作页（Cut/Merge/Editor/Workbench）
...
| 22 | probe 缓存为进程内存缓存（非文件缓存），键 = 路径+size+mtime | 文件缓存需序列化/过期/清理三件套，额外收益仅跨启动首查（冷缓存仍要扫一次）；内存方案无失效复杂度，会话内收益已覆盖"重复打开秒出"诉求 |
| 23 | e2e 落地为 cargo 集成测试（真实 sidecar 跑命令链），不引入 tauri-driver UI 自动化 | 回归风险集中在命令与参数层而非 UI 层；tauri-driver 环境重、断言脆、Windows 维护成本高；命令级 e2e 已覆盖"重构不烂核心链路"的保险目标 |
| 24 | 工作页离开时隐藏不卸载（keep-alive），设置变更一律不重置页面 | 工作台状态（素材/片段/时间轴/播放位置）重建成本高，用户明示期望保留；隐藏时自动暂停规避后台出声；确需重建的设置须二次确认（当前无此类项） |
| 25 | 深浅主题做三态：深色 / 浅色 / 跟随系统，与主题色基建同源（同一组 CSS 变量） | 用户明示需要"系统黑白色"；跟随系统是 Windows 惯例预期；不引入第二套令牌体系，避免主题 × 强调色的组合爆炸 |

---

## 16. 功能候选池（Backlog，2026-09-14 整理，未定稿）

> **状态说明**：本节是候选清单，不是承诺。条目晋升为里程碑前，需在本文件补充完整设计（数据模型/命令/UI）并过决策记录；PLAN 的候选池小节与本节同步（编号 B1…引用）。排序 = 建议优先级；量级为单人估时参考。
>
> **已晋升**：B1、B2、B14 → M8（2026-09-14，已实现）；B17 → M10-2（2026-09-15，待实施，设计见 §9.1 / 决策 #25）；下表保留原条目并标注去向。

### A. 高性价比（建议下一批）

| # | 候选 | 说明 | 量级 |
| --- | --- | --- | --- |
| B1 | ~~probe / 关键帧索引缓存~~ → M8 已实现 | 按 `路径+size+mtime` 缓存 MediaInfo 与关键帧列表；重复打开同一文件秒出（现每次进剪切页重扫数秒） | 小 |
| B2 | ~~磁盘空间预检推广到全部任务~~ → M8 已实现 | cut 已有 fs4 预检（§8.2），抽公共函数接入 merge/rotate/crop/pipeline——空间不足不再编码中途失败留半成品 | 小 |
| B3 | 缩略图 / 关键帧并行生成 | `generate_thumbnails_sync` 串行循环改线程池并行，素材多时首次加载提速 | 小 |
| B4 | 帧截图导出 | 预览画面「保存当前帧为 PNG」（canvas drawImage），与局部放大功能天然配套 | 极小 |
| B5 | 窗口状态记忆 | tauri-plugin-window-state 一行接入，记住大小/位置/最大化 | 极小 |
| B6 | 音频提取 / 音轨替换 | `-map` remux 级无损：提取音轨为 m4a/mp3、给视频换/删音轨；复用现有任务系统 | 中小 |

### B. 值得做（按需求热度）

| # | 候选 | 说明 | 量级 |
| --- | --- | --- | --- |
| B7 | Smart cut（自"远期候选"升格） | 精确剪切仅重编码切点附近 GOP，其余 stream copy——画质/速度/精确三点全占；复用 plan_items 分段思想、timescale 处理、临时令牌；难点在 GOP 解析与音画对齐 | 大 |
| B8 | 导出预设 | 一键参数档（"高兼容 1080p H.264"/"B 站投稿"等），底层 = 编码参数映射表扩展 | 中 |
| B9 | GIF / WebM 短片段导出 | 剪切区间选择 UI 全部现成，扩展输出格式；表情包高频场景 | 中小 |
| B10 | 时间轴块修剪手柄 | 工作台时间轴块两端拖动微调入出点（M6-8 遗留候选）；需处理最短时长与关键帧提示 | 中 |
| B11 | 工作台撤销栈 | Ctrl+Z 操作快照（增删片段/排序/参数变更）；Delete 误删可恢复 | 中 |
| B12 | 任务完成通知 | tauri-plugin-notification 系统通知 + 可选自动打开输出目录；批量场景价值上升 | 小 |
| B13 | 字幕提取 .srt | 精确剪切丢弃字幕流的补偿：`-map 0:s -c:s srt` 导出 | 小 |
| B17 | ~~深浅主题切换~~ → M10-2 待实施 | 亮色变量集整体替换 + 对比度校对（§14 阶段 4 既有项）；与主题色基建（§9.1）同源；三态深色/浅色/跟随系统（决策 #25） | 中 |

### C. 工程质量

| # | 候选 | 说明 | 量级 |
| --- | --- | --- | --- |
| B14 | ~~核心链路 e2e 自动化~~ → M8 已实现 | 用夹具真实跑「剪切→合并→pipeline」断言输出时长/可解码；防重构回归的保险（落地形式见 §6.6 / 决策 #23） | 中 |
| B15 | 评审遗留 P2 收尾 | 调度线程 panic 加固、16bit 高位深判定、**asset scope 收窄 + CSP（公开发布前必做）**、check_pipeline facts 缓存、expand_video_inputs 条数上限（详见 HANDOFF「Code Review」） | 中 |
| B16 | README 截图 | 界面稳定后补功能截图 | 小 |

### 不做（记录避免反复讨论）

- 界面多语言 i18n（用户群明确）
- 多轨时间线编辑器、特效/转场/字幕编辑（§1.3 非目标）
