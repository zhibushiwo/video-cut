...
- [ ] **M4-8 设置页二期**（方案 §9.9/§9.1/§12，2026-09-13 定稿）：
  - 主题色预设单选（运行时覆写 `--color-signal`；清理 `accent-[#4cc38a]` 硬编码与 `::selection`）`§9.1`
  - 缓存管理：`cache_usage`/`clear_cache` 命令（app_cache_dir 的代理与缩略图）+ 占用显示 + ask 二次确认清理 `§5.4`
  - 导出名时间戳开关（`appendTimestamp`，默认开）`§8.3`
...
- [ ] **M6-8（增强，可选）**：`generate_thumbnails` 支持 `(input, timeSec)` 生成片段起点帧缩略图；时间轴块边缘拖动微调入出点
...
- [ ] **M7-7（暂缓）拖入文件夹**：Rust `expand_video_inputs`（目录递归扫视频扩展名、跳过隐藏目录、排序）
- [ ] **M7-8（暂缓）安装包中文**：NSIS `languages: ["SimpChinese"]`；MSI 本地化弱，倾向移除只留 NSIS（M4-5 验收同步）
- [ ] **M7-9（暂缓）新图标**：BrandMark 矢量稿（墨底圆角方 + signal 绿切块）→ resvg 渲染 1024px → `pnpm tauri icon` 全量生成
