...
- [ ] **M2-1 参数一致性比对**：`commands/merge.rs` 实现九项比对（DESIGN §3.3 清单），比对函数独立可单测
- [ ] **M2-2 检测面板 UI**：文件列表 + 摘要列（编码/分辨率/帧率）+ ✓/⚠ 面板与差异项高亮 `§9.5`
- [ ] **M2-3 无损合并**：concat 列表生成（UTF-8、单引号转义、正斜杠）+ `build_merge_command` + 单测 `§6.3③`
- [ ] **M2-4 拖拽排序**：列表手柄拖拽（原生 drag events，不引库）
- [ ] **M2-5 自动统一路径**：逐个转中间文件（`build_normalize_command`）→ 二次 concat；目标参数选择 UI（首文件参数 / 1080p H.264）；二次确认弹窗 `§6.3④`
- [ ] **M2-6 音轨/字幕布局校验**：流布局不一致时的明确报错文案 `§3.3`
...
- [ ] **M4-1 设置页**：输出目录、默认模式、吸附开关、代理开关/强制代理、编码器锁定、质量档位；读写 tauri-plugin-store `§12`
- [ ] **M4-2 历史记录**：任务历史 JSON 追加存储 + 历史页（重新定位输出文件）`§12`
