...
| **M8 候选池晋升批次 1** | B1 probe 缓存、B2 磁盘预检推广、B14 核心链路 e2e | M6 | 0.5~1 天 | cargo test 全绿含 e2e；同文件二次打开秒出 |
| **M9 工作台修复冲刺** | 手测批次 2 的 4 个 bug + 片段区间预览（成品预览/切换重置/时间轴布局与拖拽） | M6 | 0.5~1 天 | 成品可连播；切换素材出入点重置；时间轴块不重叠且整块可拖 |
| **M10 保活 + 深浅主题** | 工作页 keep-alive（决策 #24）+ 深浅主题三态（B17，决策 #25） | M9 | 1 天 | 去设置/历史回来状态全保留；深浅色切换含跟随系统 |
...
- [x] **M4-3 快捷键**：空格播放/暂停、←/→ ±1 秒、Shift+←/→ 逐帧（1/fps）、I/O 设入/出点、Delete 删片段——剪切页 + 工作台（成品/源剪切/片段加工三态）全覆盖；`useHotkeys` 忽略输入焦点，空格/方向键 preventDefault 防滚动 `§9.4`
...
> 按 DESIGN §16 晋升规则执行：设计已在 §6.5（probe 缓存）/ §6.6（e2e）/ §8.2（磁盘预检）定稿，决策 #22/#23 已过。同一文件"重复打开秒出"（B1）+ 磁盘不足提前失败（B2）+ 重构回归保险（B14）。
...
## M9 工作台修复冲刺（手测批次 2，2026-09-15 立项，待实施）

> 根因诊断见 HANDOFF「M9/M10 反馈诊断」。7 项反馈中 4 个 bug + 1 个体验需求入本批，保活与主题入 M10。

- [ ] **M9-1 [P0] 成品预览修复**：ProductPreview 的 `<video src>` 一律经 `fileSrc()`（convertFileSrc 资源协议，含代理路径）——当前用裸磁盘路径 WebView 加载不了，成品完全不可播（§9.8 修复约定）
- [ ] **M9-2 [P1] 切换重置**：CutModeView/EditModeView 渲染点按业务 id 加 `key`（sourceId/clipId），切换素材/片段时进度、出入点、关键帧状态全部重建
- [ ] **M9-3 [P1] 时间轴布局**：块位置改逐块像素排布（容器实测宽换算），最小宽度块的下一段起点 = 前块实际右缘，消除"百分比 left + minWidth 28"重叠
- [ ] **M9-4 [P1] 时间轴整块拖拽**：块本体 onPointerDown 进拖拽（复用 4px 死区，点击选中不受影响），取消 6px 独立手柄；与 M9-3 同组件同批提交
- [ ] **M9-5 [P2] 片段区间内预览**：加工视图播放限制在片段区间，越过出点自动暂停（循环开关可选）（§9.8）

**验收**：成品预览可从头连播到尾并可 seek；切换素材/片段无残留状态；多短片段时间轴块互不压盖、拖块即可排序；加工视图播完出点即停。

## M10 保活 + 深浅主题（2026-09-15 立项，待实施）

- [ ] **M10-1 [P1] 页面保活**（决策 #24）：Cut/Merge/Editor/Workbench 离开隐藏不卸载，隐藏瞬间自动暂停播放；History/Settings 维持条件挂载；设置变更即时生效不重置页面（§9.2）
- [ ] **M10-2 [P2] 深浅主题**（B17，决策 #25）：`themeMode: dark | light | system` 设置项；`.theme-light` 变量组整体替换；跟随系统 matchMedia 监听实时切换；warn 琥珀与四个主题色浅底对比度逐一校对（§9.1）

**验收**：去历史/设置再回来，工作台素材/片段/时间轴/播放位置全保留，隐藏期间不发声；深浅色 + 跟随系统三态切换即时生效，浅色下全部徽标/文字可读。

**实施顺序建议**：M9-1 → M9-2 → M9-3/4 → M9-5 → M10-1 → M10-2（M10-1 若用户反馈强烈可提前到 M9-5 之前）。

## 候选池（未排期，2026-09-14 整理）

> 完整说明见 DESIGN §16（编号 B1…B17 一一对应）。**候选不是承诺**：晋升为里程碑前需先在 DESIGN 定稿设计并过决策记录。排序 = 建议优先级。B1/B2/B14 已随 M8 完成，B17 已晋升 M10（待实施），其余未排期。

- 候选 **A 高性价比**：B3 缩略图/关键帧并行 · B4 帧截图导出 · B5 窗口状态记忆 · B6 音频提取/音轨替换（~~B1 B2 已随 M8 完成~~）
- 候选 **B 值得做**：B7 Smart cut · B8 导出预设 · B9 GIF/WebM 短片段 · B10 时间轴块修剪手柄 · B11 工作台撤销栈 · B12 任务完成通知 · B13 字幕提取（~~B17 已晋升 M10~~）
- 候选 **C 工程质量**：B15 评审遗留 P2 收尾（含发布前必做的 asset scope 收窄）· B16 README 截图（~~B14 已随 M8 完成~~）
- 不做：i18n、多轨编辑器（DESIGN §1.3 / §16）

**建议下一批组合**：M9/M10 落地后，功能面优先 B6 音频提取 + B12 任务通知，工程面优先 B15（发布前 asset scope 收窄是硬门槛）。
