...
> 完整说明见 DESIGN §16（编号 B1…B17 一一对应）。**候选不是承诺**：晋升为里程碑前需先在 DESIGN 定稿设计并过决策记录。排序 = 建议优先级。

- 候选 **A 高性价比**：B1 probe/关键帧缓存 · B2 磁盘预检推广 · B3 缩略图/关键帧并行 · B4 帧截图导出 · B5 窗口状态记忆 · B6 音频提取/音轨替换
- 候选 **B 值得做**：B7 Smart cut · B8 导出预设 · B9 GIF/WebM 短片段 · B10 时间轴块修剪手柄 · B11 工作台撤销栈 · B12 任务完成通知 · B13 字幕提取 · B17 深浅主题切换
- 候选 **C 工程质量**：B14 核心链路 e2e 自动化 · B15 评审遗留 P2 收尾（含发布前必做的 asset scope 收窄）· B16 README 截图
- 不做：i18n、多轨编辑器（DESIGN §1.3 / §16）

**建议下一批组合**：B1 + B2 + B14（两次使用体感 + 回归保险），功能面优先 B6 / B7。
