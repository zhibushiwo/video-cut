...
- [ ] **M8-1 probe/关键帧缓存（B1）**：probe.rs 五个探测入口（media/facts 异步 + facts/duration 同步 + 关键帧）进程内缓存，键 = 路径+size+mtime_ns，仅缓存成功结果；单测覆盖键失效与命中
- [ ] **M8-2 磁盘预检推广（B2）**：抽 `require_disk_space` 公共函数，cut 改造接入，rotate/crop/merge/pipeline/proxy 全部接入（估算规则见 DESIGN §8.2 表）
...
> 完整说明见 DESIGN §16（编号 B1…B17 一一对应）。**候选不是承诺**：晋升为里程碑前需先在 DESIGN 定稿设计并过决策记录。排序 = 建议优先级。B1/B2/B14 已晋升 M8，其余未排期。

- 候选 **A 高性价比**：B3 缩略图/关键帧并行 · B4 帧截图导出 · B5 窗口状态记忆 · B6 音频提取/音轨替换（~~B1 B2 已晋升 M8~~）
- 候选 **B 值得做**：B7 Smart cut · B8 导出预设 · B9 GIF/WebM 短片段 · B10 时间轴块修剪手柄 · B11 工作台撤销栈 · B12 任务完成通知 · B13 字幕提取 · B17 深浅主题切换
- 候选 **C 工程质量**：B15 评审遗留 P2 收尾（含发布前必做的 asset scope 收窄）· B16 README 截图（~~B14 已晋升 M8~~）
- 不做：i18n、多轨编辑器（DESIGN §1.3 / §16）

**建议下一批组合**：B6 + B12（功能体感），工程面优先 B15（发布前 asset scope 收窄是硬门槛）。
