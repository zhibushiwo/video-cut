...
| **R3 收尾** | probe 缓存改 LRU、取消清理按 kind 统一、snapshot 过滤 internal、speed 接通、输出前导抽取、锁策略 | M11 | 1~2 天 | 🔜 **部分完成**：`R3-7`（输出=输入同一性比较归一化）已提前实施（2026-09-24）、`R3-1`（probe 缓存 LRU）、`R3-2`/`R3-3`（任务去重统一管理 + internal 不可见）、`R3-4`（节流收敛 + speed/ETA 接通）已完成（2026-09-26）；仅剩 `R3-5`/`R3-6` |
