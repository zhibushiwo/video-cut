...
<!-- check-docs:skip -->
> ⚠ **豁免原因**：下表「提交信息」列属**历史引文**（照录当年的提交原文）——其中 `DESIGN §17` / `DESIGN §18` 按**拆分前的 DESIGN** 计数（现分别位于 TIMELINE.md §17 与 plans/M11.md），`§11` 指 DESIGN.md §11。按归档「只追加不改写」规则原文保留，故下表不参与文档一致性检查（登记见 [INDEX.md](../INDEX.md) §7.1）。
