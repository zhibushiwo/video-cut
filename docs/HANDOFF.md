...
| **R4 第二轮审查整改**（7 条，修 `BUG-001`~`006`；另 `R4-8` 修真机首跑缺陷 `BUG-007`~`009`） | 🔜 **下一步**：`R4-1`–`R4-7` 待做；**`R4-8` 已实施**（2026-09-21） |
| M11 单轨时间线核心 → M12 预览强化 → M13 打磨 | ⏳ R4 之后（M11-0~M11-9 未开工） |

## 下一步（按序）

**R4-1~R4-7（审查整改）→ M11-0（状态层/撤销基座，吸收 R2）→ M11-1~M11-9 → R3 → M12-2 → M12-1/3 → M13**；M10 与时间线核心零耦合，视反馈随时插入。

- **已做的最近一批**（2026-09-21，`R4-8`）：真机首跑抓出的 `BUG-007`（吸附值取整导致成品比界面长一个 GOP）/ `BUG-008`（关键帧丢项）/ `BUG-009`（不显示实际落点）三条已修复；`BUG-007`/`BUG-009` 真机复验通过转 `verified`，`BUG-008` 代码已修 + 解析单测已锁，**真机复跑 `TC-030` 待发起**。**未提交**（待用户定提交切分）。
