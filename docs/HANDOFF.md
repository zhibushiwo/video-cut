...
| 7 | 需要系统黑白色 | 功能 | 即候选池 B17 晋升：三态深色/浅色/跟随系统（决策 #25，设计 §9.1） | P2（M10-2） |

合并关系：#2+#4 同根（ClipTimeline 布局+手柄，一个提交组）；#3 依赖 #5 修好后的预览体系；#6/#7 相互独立。

## 待办

1. **M9/M10 实施**（已立项待用户指令，优先级与根因见上表）：M9-1 成品预览 → M9-2 切换重置 → M9-3/4 时间轴 → M9-5 片段区间预览 → M10-1 保活 → M10-2 深浅主题
2. **用户统一手测**：M7-1/2/3 bug 修复、第一批快改、工作台 2.0 全流程（M9/M10 落地后一并复验本轮 7 项）、快捷键（剪切页与工作台各模式）、M4-8（主题色/缓存/关闭确认/重置与关于）；**M4-5 实机冒烟**——NSIS 包装干净环境，验证中文向导/新图标/全功能/SmartScreen
3. 遗留小项：规则 B 下"有片段裁剪 + 其他片段非恒等旋转"时后者也转码（已文档化）；硬编路径未在真 GPU 上验证；旋转覆盖源 flip 元数据（罕见）；代理关闭时不支持格式仅显示提示条；日志跨天不切文件；时间轴块边缘拖动微调未做（M6-8 未列入的剩余候选）；README 截图待补
4. 功能候选池：DESIGN §16 / PLAN 候选池（B3–B13/B15/B16 未排期；B1/B2/B14 已随 M8 完成，B17 已晋升 M10）；功能面优先 B6 音频提取 + B12 任务通知，工程面优先 B15（发布前 asset scope 收窄是硬门槛）；i18n 已明确不做
