...
| M6 工作台 2.0（多片段/片段池/合成时间轴/成品连播） | 方案定稿（DESIGN v0.3）；M6-0 落地页改造 ✅（待提交），M6-1~M6-8 未开始 | — |
...
- `appendTimestamp` 开关（默认 true）：`utils/time.ts::withFileTimestamp` 的四个调用点按设置走
...
1. 用户手测：拖入文件不再跳页（OS 级拖拽无法自动化模拟）；设置页改动重启后保留；落地页右上角导航跳转
2. M7 余项：M7-4~M7-9 暂缓（同名才加时间戳/浮层自动关闭/预览倍速/拖入文件夹/NSIS 中文/新图标，方案见 PLAN M7）
3. M6 余项：M6-1~M6-5（数据模型/卡片/池/时间轴/导出）→ M6-6（成品连播）→ M6-7（验证）；M6-8 可选
4. M4 余项：M4-3 快捷键、M4-4 批量处理、M4-5 NSIS/MSI 安装包实机验证、M4-6 文档收尾（README 仍是模板内容）、M4-8 设置页二期（方案已定稿，见上）、**M4-7 日志系统（最后一项，§12.1）**
