...
- **已做的最近四批**（2026-09-21）：
  ...
  - **`R4-1`** 作业体 panic 隔离（`run_job_isolated`）——`BUG-001` 转 `verified`，`TC-019` 通过。连带的「发布版 panic 策略」已裁决：release 保留 `panic = "unwind"`（[DECISIONS.md](./DECISIONS.md) `ADR-034`，原 `abort` 会让隔离在打包后失效）。**未提交**。
  - **`R4-2`** 输出原子替换（`fs::atomic_replace` 6 处）——`BUG-002` 转 `fixed`，`TC-020` 通过。**未提交**（与 `R4-1` 同批待提交）。
  - **`R4-3`** 指针拖拽收尾兜底（新增 `utils/pointerDrag`，四处拖拽共用）——`BUG-003` 转 `fixed`，`TC-021` **手工待跑**（需真机拖出窗口松手，**待用户发起**）。**未提交**。
