...
| **R4 第二轮审查整改**（7 条，修 `BUG-001`~`006`；另 `R4-8` 修真机首跑缺陷 `BUG-007`~`009`） | 🔜 **进行中**：`R4-1` / `R4-2` / `R4-3` / `R4-8` 已实施（2026-09-21），`R4-4`–`R4-7` 待做 |
| M11 单轨时间线核心 → M12 预览强化 → M13 打磨 | ⏳ R4 之后（M11-0~M11-9 未开工） |

## 下一步（按序）

**R4-4~R4-7（审查整改）→ M11-0（状态层/撤销基座，吸收 R2）→ M11-1~M11-9 → R3 → M12-2 → M12-1/3 → M13**；M10 与时间线核心零耦合，视反馈随时插入。

- **已做的最近四批**（2026-09-21）：
  - **`R4-8`** 真机首跑抓出的三缺陷（`BUG-007` 吸附值取整导致成品比界面长一个 GOP / `BUG-008` 关键帧丢项 / `BUG-009` 不显示实际落点）——`BUG-007`/`BUG-009` 真机复验通过转 `verified`，`BUG-008` 解析单测已锁、**真机复跑 `TC-030` 待发起**。已提交（`fdff11b` + `627cbfc`）。
  - **`R4-1`** 作业体 panic 隔离（`run_job_isolated`）——`BUG-001` 转 `verified`，`TC-019` 通过。连带的「发布版 panic 策略」已裁决：release 保留 `panic = "unwind"`（[DECISIONS.md](./DECISIONS.md) `ADR-034`，原 `abort` 会让隔离在打包后失效）。
  - **`R4-2`** 输出原子替换（`fs::atomic_replace` 6 处）——`BUG-002` 转 `verified`，`TC-020` 通过。
  - **`R4-3`** 指针拖拽收尾兜底（新增 `utils/pointerDrag`，四处拖拽共用）——`BUG-003` 转 `fixed`，`TC-021` **手工待跑**（需真机拖出窗口松手，**待用户发起**）。
  - 上述三批已提交：`a783cca` · `3137072` · `e5a2f43` · `5aeffd4`（ADR-034）· `f278214`（docs）。
- **待用户统一手测**：M4-5 NSIS 干净环境冒烟 · M7-1/2/3 · 工作台 2.0 全流程 · 快捷键 · M4-8 · **M9 五项**（连播可 seek / 切换无残留 / 多短片段不压盖且整块可拖 / 加工视图播完出点即停） · **`TC-030` 复跑**（关键帧刻度条数 == ffprobe 原始行数）
