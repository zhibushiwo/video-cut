...
| M6 工作台 2.0（多片段/片段池/合成时间轴/成品连播） | M6-0~M6-5 ✅（素材/片段/时间轴 + 三态预览 + 导出衔接）；M6-6 连播、M6-7 e2e、M6-8 增强未做 | 本批提交 |
...
1. 用户手测：M7-1/2/3 三项修复（非视频拖入遮罩消失 / 放大页可播放 / 列表拖拽排序）+ 第一批（导出同名才加时间戳 / 浮层自动关闭 / 预览倍速 / 拖入文件夹）；**工作台 2.0**（拖入多素材 → 点卡剪切出多片段 → 池卡加工旋转/放大 → 时间轴拖拽排序/插入 → 合成导出验证输出正确）
2. 第二批余项：M6-6 成品连播预览 → M6-7 e2e 验证 → M6-8 增强（批量能力 = 原 M4-4）；M4-3 快捷键（M6-4 后接入，现可开始）
