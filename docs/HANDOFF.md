...
| R2 重构 · R3 收尾 | 🔄 R2 进行中（`R2-1` ✅ 2026-09-24；`R2-2` ✅ 2026-09-25；**`R2-3` useHotkeys 激活门控 ✅ 2026-09-25**；`R2-4`–`R2-5` 待做）；R3 在 M12-2 前——例外：**`R3-7` 已提前实施（2026-09-24）** |
...
**`R2-4`~`R2-5`（ESLint 基线与 `noUncheckedIndexedAccess`、死代码清理）→ `M11-1`~`M11-9` → R3 → M12-2 → M12-1/3 → M13**；M10 与时间线核心零耦合，视反馈随时插入。**R4 段 9 条已全部实施完毕**（2026-09-21 ~ 09-23）；**`M11-0`、`R2-1`~`R2-3` 已实施**（2026-09-24 ~ 09-25）。
...
- **已做的最近十二批**：
  ...
  - （2026-09-23）**`R4-4`** `pxToCrop` 锚点钳制（锚点上界与收缩步都走 `evenFloor` 向下取偶）——`BUG-004` 转 `fixed`；边界矩阵用**命令级脚本**跑通（14 组期望值 + 4000 组随机扫描 / 12730 断言 0 失败；同矩阵在修复前实现上越界 993/1554），矩阵含期望值与扫描域/种子已记入 [TESTING.md](./TESTING.md) TC-022，**转写为 Vitest 用例的工作见 PLAN `T-004`**（载体已由 `M11-0` 就位）；**真机手工（数值微调）待跑**。
...
| e2e 断言强度未评估 | 评审 §8 · §5.3 | `tests/e2e.rs` 断言强度未逐条评估；覆盖缺口：rotate、crop_zoom、merge 非兼容转码分支、proxy 去重 | 回归保障强度 | 需先补齐 R4 回归 TC 才有对照基线 | **R4 已完成（2026-09-23，TC-019–TC-029 就位）、Vitest 载体也已就位（`M11-0`，2026-09-24）——本项已到复查点**；评估时把 `proxy` 去重与 `merge` 非兼容转码分支的缺口一并纳入（可与 `TC-022`/`TC-024`/`TC-028` 的命令级矩阵一起转成 Vitest 用例） |
